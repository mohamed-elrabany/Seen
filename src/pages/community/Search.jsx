...
export default function Search() {
      ...
      try {
        ...
        setResults((prev) => {
          const newPosts =
            page === 1
              ? response.posts.data
              : [...prev.posts, ...response.posts.data];
          const newUsers =
            page === 1 ? response.users : [...prev.users, ...response.users];

          return {
            // Filter posts by unique post_id
            posts: newPosts.filter(
              (item, index, self) =>
                index === self.findIndex((t) => t.id === item.id),
            ),
            // Filter users by unique id
        ...
        // 3. Proper hasMore Logic
        // If the current category's returned data is empty or short, stop pagination
        const currentData =
          searchCategory === "posts" ? response.posts.data : response.users;
        if (
          !currentData ||
  ...
  }, [query, page, searchCategory]);

  // 3. Scroll Handler for the Modal's Internal Div
  const onScroll = () => {
    if (scrollRef.current) {
      const { scrollTop, scrollHeight, clientHeight } = scrollRef.current;
      // check !isLoading strictly
      if (
        scrollHeight - scrollTop <= clientHeight + 25 &&
        !isLoading &&
        hasMore
      ) {
        setIsLoading(true); // Lock it immediately
        setPage((prev) => prev + 1);
      }
    }
  };
          ...
          <div className="p-6 border-b dark:border-white/5 bg-gray-50/50 dark:bg-white/5 flex-shrink-0">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-3">
                ...
                <h3 className="text-lg font-bold dark:text-white mb-0">
                  {t("Search")}
                </h3>
              </div>
              <button
                onClick={onClose}
                className="text-gray-400 hover:scale-110 transition-all cursor-pointer"
              >
