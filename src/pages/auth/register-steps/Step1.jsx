...
export default function Step1({ data, setData, isStepValid }) {
  ...
  // Only runs once on mount — handles back-navigation case
  useEffect(() => {
    if (initialCheckDone.current) return;
    const isValidFormat = emailRegex.test(data.email);
    if (isValidFormat && !isEmailVerified && emailStatus === "idle") {
  ...
  return (
    <div className="">
      <h2 className="mb-6">{t("registerPage.step1.title")}</h2>
      ...
      <div className="grid gap-4">
        ...
        <Input
          ...
          error={
            touched.phone && data.phone && !phoneRegex.test(data.phone)
        ...
        <Input
          ...
          error={
            touched.password && data.password && data.password.length < 8
