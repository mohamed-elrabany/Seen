...
export default function CommentsFeed({ comments, isLoading, moreComments, onLikeComment }) {

  return (
    <div className="w-full mt-8 pb-8 space-y-4">
      {comments.map((comment, index) => (
        // Note: Better to use comment.id than index if available
        <PostComment key={comment.id} comment={comment} onLike={() => onLikeComment(comment.id)} />
      ))}
